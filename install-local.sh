...
# Quick install script for Tbilisi Night theme in Cursor

EXTENSION_DIR="$HOME/.cursor/extensions"
THEME_DIR="$EXTENSION_DIR/tbilisi-night"
CURRENT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo "🎨 Installing Tbilisi Night theme locally in Cursor..."
...
if [ $? -eq 0 ]; then
    ...
    echo "2. Select theme (Cmd+K Cmd+T → 'Tbilisi Night')"
    echo ""
